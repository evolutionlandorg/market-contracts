...
// Use proxy mode
contract RevenuePoolV5 is DSAuth, AuctionSettingIds {
...
//    address public devPool;
    ...
    function settleToken(address _tokenAddress) public {
        ...
        if (balance > 100) {
            ...
            address farmPool = registry.addressOf(AuctionSettingIds.CONTRACT_FARM_POOL);
            ...
            require(pointsRewardPool != 0x0 && contributionIncentivePool != 0x0 && farmPool != 0x0 && governorPool != 0x0  && devPool != 0x0, "invalid addr");
            ...
            if (IGovernorPool(governorPool).checkRewardAvailable(_tokenAddress)) {
                ...
                IGovernorPool(governorPool).rewardAmount(balance * 30 / 100);
            }
