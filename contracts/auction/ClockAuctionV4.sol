...
contract ClockAuctionV4 is PausableDSAuth, AuctionSettingIds {
    using SafeMath for *;
    ...
    */
    ...
    function _secondPartBid(uint _auctionCut, address _pool, address _buyer, Auction storage _auction, uint _priceInToken, address _referer) internal returns (uint, uint){
        ...
        uint realReturnForSeller = (surplus - poolCutAmount) * 80 / 100;
        uint realReturnForLastBidder = (surplus - poolCutAmount) * 20 / 100;
        uint returnToLastBidder = realReturnForLastBidder + uint256(_auction.lastRecord);
        // here use transfer(address,uint256) for safety
        _sellerPay(_auction.token, _pool, _auction.seller, _buyer, realReturnForSeller);
