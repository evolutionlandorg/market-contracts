...
contract PointsRewardPoolV3 is PausableDSAuth, AuctionSettingIds {
    using SafeMath for *;
    ...
    function playWithSmallTicket() public isHuman whenNotPaused {
        uint fee = registry.uintOf(UINT_SMALL_TICKET_FEE);
        _play(fee, 8);
    }

    function playWithLargeTicket() public isHuman whenNotPaused {
        uint fee = registry.uintOf(UINT_LARGE_TICKET_FEE);
        _play(fee, 10);
    }
